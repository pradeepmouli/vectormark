...
def _three_facets(h, w):
    """4-quadrant flat facets (strong, non-monotone colors): no single parametric gradient
    can fit more than half the pixels, so the median per-pixel ΔE > _SMOOTH_MEDIAN_TOL even
    with the band-count guard bypassed."""
    yy, xx = np.mgrid[:h, :w]
    img = np.zeros((h, w, 3), np.uint8)
    img[(yy < h // 2) & (xx < w // 2)] = (220, 30, 30)     # top-left: red
    img[(yy < h // 2) & (xx >= w // 2)] = (30, 30, 220)    # top-right: blue
    img[(yy >= h // 2) & (xx < w // 2)] = (30, 200, 30)    # bottom-left: green
    img[(yy >= h // 2) & (xx >= w // 2)] = (220, 180, 30)  # bottom-right: yellow
    return img
